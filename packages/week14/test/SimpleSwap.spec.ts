...
import { SimpleSwap, SimpleSwap__factory, TestERC20 } from '../test-types';
...
describe('SimpleSwap Spec', () => {
	...
	describe('# constructor', () => {
		...
		it('forces error, when tokenA is not a contract', async () => {
			await expect(
				simpleSwapFactory.deploy(ethers.constants.AddressZero, tokenB.address),
			).to.be.revertedWith('SimpleSwap: TOKENA_IS_NOT_CONTRACT');
		});

		it('forces error, when tokenB is not a contract', async () => {
			await expect(
				simpleSwapFactory.deploy(tokenA.address, ethers.constants.AddressZero),
	...
	describe('# removeLiquidity', () => {
		...
		it('forces error, when lp token amount is zero', async () => {
			await expect(
				simpleSwap.connect(maker).removeLiquidity(parseUnits('0', slpDecimals)),
